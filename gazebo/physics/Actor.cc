...
void Actor::AlignBVH(Skeleton* _skel, std::map<std::string, std::string> _skelMap){
  ...
  for (unsigned int i = 0; i < _skel->GetNumNodes(); ++i)
  {
      ...
      // std::cerr << animNode->GetChildCount() << "544 \n";
      ...
      // std::cerr << animNode->GetChild(0)->GetName() << "565 \n";
      ...
      // std::cerr << animNode->GetChild(0)->GetName() << "584 \n";
    ...
    std::map<std::string, std::string> _skelMap, const double _time)
{
  ...
  for (unsigned int i = 0; i < this->skeleton->GetNumNodes(); ++i)
  {
    ...
    if (!parentBone)
    {
      bone_pose->mutable_position()->CopyFrom(msgs::Convert(ignition::math::Vector3d()));
      bone_pose->mutable_orientation()->CopyFrom(msgs::Convert(ignition::math::Quaterniond()));
      if (!this->customTrajectoryInfo)
        mainLinkPose = bonePose;
    }
    else
    {
      // std::cerr << bone->GetName() << ": " << bonePose.Pos() << "\n";

      // bone_pose->mutable_position()->CopyFrom(msgs::Convert(temp));
  ...
  {
    model_pose->mutable_position()->CopyFrom(msgs::Convert(this->worldPose.Pos()));
    model_pose->mutable_orientation()->CopyFrom(msgs::Convert(this->worldPose.Rot()));
  }
